...
void fillTriangle_EdgeWalking_Z_UV(Vec3i t0, Vec3i t1, Vec3i t2, Vec2i uv0, Vec2i uv1, Vec2i uv2, TGAImage& image, float intensity, float* zbuffer)
{
    if (t0.y == t1.y && t0.y == t2.y) return; // i dont care about degenerate triangles
    ...
    { // draw the model
        ...
        for (int i = 0; i < model->nfaces(); i++)
        {
            ...
            if (intensity > 0) 
            {
                ...
                //重头戏，插值出每一个点的信息
                fillTriangle_EdgeWalking_Z_UV(screen_coords[0], screen_coords[1], screen_coords[2], uv[0], uv[1], uv[2], image, intensity, zbuffer);
            }
        }
