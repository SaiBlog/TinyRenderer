...
//可以使用vec<DIM,T>来声明一个我们想要的结构
template <size_t DIM, typename T> 
struct vec 
{
    vec() { for (size_t i = DIM; i--; data_[i] = T()); }//调用T的默认构造函数
    T& operator[](const size_t i) { assert(i < DIM); return data_[i]; }//定义类似与数组下标的访问符
    const T& operator[](const size_t i) const { assert(i < DIM); return data_[i]; }
private:
    T data_[DIM];//真正存储数据的地方
};
